...
const prodLogsDir = path.join(artifactsDir, 'prod-logs');
const DEFAULT_ENV = {
...
async function findAvailablePort() {
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const candidate = 4200 + Math.floor(Math.random() * 1000);
  ...
  throw new Error('Unable to find an available port for the production server.');
}

const READY_PATTERNS = [/ready - started server/i, /started server on/i, /ready in/i, /listening on/i];
...
function createLogCollector(mode) {
  ...
  const append = (source, data) => {
    ...
    logs.push({ timestamp: new Date().toISOString(), source, text });
...
function startNextServer(mode, port) {
  ...
  const args = [nextCli, 'start', '--hostname', '127.0.0.1', '--port', String(port)];
  ...
  child.on('exit', (code, signal) => {
    if (!readySettled) {
      rejectReady(new Error(`Next.js production server exited before becoming ready (code ${code ?? 'null'}, signal ${signal ?? 'null'})`));
    }
  });
...
async function persistLogs(logs, directory) {
  ...
  const content = logs.map((entry) => `[${entry.timestamp}] [${entry.source}] ${entry.text}`).join('');
...
async function main() {
  ...
  await runCommand(getPnpmCommand(), ['build'], {
    env: withDefaultEnv({ NEXT_TELEMETRY_DISABLED: '1', NODE_ENV: 'production' }),
  });
  ...
  const server = startNextServer('prod', port);
  ...
  try {
    await server.ready;
    console.log('Production server is ready. Ensuring Playwright browsers are installed...');
    await ensurePlaywrightBrowsers();
    console.log('Running Playwright crawl against production build...');
    ...
    if (logErrors.length > 0) {
      logFilePath = await persistLogs(server.logs, prodLogsDir);
      throw new Error(
        `Detected ${logErrors.length} error lines in production server logs. See ${path.relative(projectRoot, logFilePath)} for details.`
      );
    }
    console.log('Production crawl completed successfully without detected runtime errors.');
  } catch (error) {
    if (!logFilePath) {
      try {
        logFilePath = await persistLogs(server.logs, prodLogsDir);
        console.error(`Preserved production server logs at ${path.relative(projectRoot, logFilePath)}`);
      } catch (persistError) {
        console.error(`Failed to persist production logs: ${persistError instanceof Error ? persistError.message : persistError}`);
      }
    }
    throw error;
  } finally {
