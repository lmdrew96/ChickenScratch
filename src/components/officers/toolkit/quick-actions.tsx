        ...
        {actions.map((action) => {
          ...
          return (
            <Link
              key={action.label}
              href={action.href ?? '#'}
              className="rounded-xl border border-white/10 bg-white/5 p-3 sm:p-4 hover:bg-white/10 hover:border-[var(--accent)]/30 transition-all hover:-translate-y-1 block min-h-[64px]"
            >
              <div className="flex items-center gap-3 mb-1">
                {Icon && <Icon className="h-5 w-5 text-[var(--accent)] shrink-0" />}
